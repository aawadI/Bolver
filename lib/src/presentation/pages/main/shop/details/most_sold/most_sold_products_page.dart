...
import 'riverpod/provider/most_sold_provider.dart';
import '../../../../../components/components.dart';
import '../../../../../../core/constants/constants.dart';

class MostSoldProductsPage extends ConsumerStatefulWidget {
  final int? shopId;
  const MostSoldProductsPage( {Key? key,this.shopId,}) : super(key: key);
...
class _MostSoldProductsPageState extends ConsumerState<MostSoldProductsPage> {
    ...
    _scrollController.addListener(
      () {
        if (_scrollController.position.maxScrollExtent ==
          ...
          ref.read(mostSoldProvider.notifier).fetchMoreProducts(shopId: widget.shopId);
        }
      },
    );
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => ref.read(mostSoldProvider.notifier).initialFetchProducts(shopId: widget.shopId),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(mostSoldProvider);
    final event = ref.read(mostSoldProvider.notifier);

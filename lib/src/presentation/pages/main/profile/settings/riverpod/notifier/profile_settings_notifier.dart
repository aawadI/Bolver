...
class ProfileSettingsNotifier extends StateNotifier<ProfileSettingsState> {
  ...
  void setNewPassword(String value) {
    state = state.copyWith(newPassword: value.trim(),isPasswordInvalid: false);
  }

  void setConfirmPassword(String value) {
    state = state.copyWith(confirmPassword: value.trim(),isConfirmPasswordInvalid: false);
  }
  ...
  Future<void> updateGeneralInfo(BuildContext context) async {
    ...
    if (connected) {
      ...
      final response = await _userRepository.updateGeneralInfo(
        ...
        birthdate: state.dob,
        gender: state.male,
        phone: (state.isPhoneEditable && state.phone.isNotEmpty)
            ? state.phone
            : null,
      );
      response.when(
  ...
  Future<void> updatePassword(BuildContext context) async {
    final connected = await AppConnectivity.connectivity();
    if (!AppValidators.isValidPassword(state.newPassword)) {

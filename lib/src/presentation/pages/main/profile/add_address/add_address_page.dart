...
class AddAddressPage extends ConsumerStatefulWidget {
  ...
  const AddAddressPage({Key? key, required this.isRequired, this.onEditID})
      : super(key: key);
...
class _AddAddressPageState extends ConsumerState<AddAddressPage>
  ...
  void initState() {
    ...
    Future.delayed(
      Duration.zero,
      () {
        if (widget.onEditID != null) {
          ref
              .read(addAddressProvider.notifier)
              .fetchShowLocation(widget.onEditID ?? 0);
        }
      },
    );
  }
                    ...
                    children: [
                          ...
                          state.isSearching
                              ...
                              : ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    final location =
                                        state.searchedLocations[index];
                                    return SearchedLocationItem(
                                      location: location,
                                      onTap: () {
                                        FocusManager.instance.primaryFocus
                                            ?.unfocus();
                                        notifier.goToLocation(place: location);
                                      },
                                    );
                                  },
                                ),
                      ...
                      AnimatedPositioned(
                        ...
                        child: AccentLoginButton(
                          ...
                          onPressed: state.location?.longitude == null
                              ? null
                              : () {
                                  AppHelpers.showCustomModalBottomSheet(
                                    context: context,
                                    modal: EnterLocationTitleModal(
                                      isEdit: widget.onEditID != null,
                                      hasBack: !widget.isRequired,
                                    ),
